...
4. загрузит API, соберёт frontend и запустит Compose;
5. дождётся healthcheck обоих контейнеров и напечатает адреса и команды управления.
...
Повторный запуск безопасен: существующие `.env`, `cookies.json` и данные конфигурации не перезаписываются. Файлы Compose/Dockerfile обновляются из текущего комплекта.
...
Если `CTID` не задан, берётся следующий свободный ID кластера. Скрипт не удаляет уже существующие LXC и завершится с ошибкой при совпадении ID. После установки он выведет URL и команду `pct enter CTID`.
...
- **YouTube отвечает bot/403:** это ограничение YouTube/IP. Upstream также поддерживает `yt-session-generator`, но он требует отдельного обслуживания и намеренно не включён в минимальный комплект.
